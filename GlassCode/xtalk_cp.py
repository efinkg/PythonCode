...
class Bot:
    ...
    def message_analyzer(self,body):	#handle incoming messages
        if (body.find('temperature')>=0):
            ...
            message = xmpp.Message(source, "/push CPU temperature of Pi is: "+temperature_result[5:])
        ...
        if (body.find('coffee')>=0):
	   size = 16
	   try:
	      size = int(request.form['size'])
    	   except:
	      pass # if it isn't an int, or data wasn't sent
	   message = xmpp.Message(target, 'I am making you %d ounces of coffee.')
	   message.setAttr('type', 'chat')
           bot.send(message)
...
if __name__ == '__main__':
   ...
   #     sys.exit(0)
    ...
    source = 'coffeemaker@jabber.iitsp.com'		#change it to your own
    password = 'Coffee'					#change it to your own
